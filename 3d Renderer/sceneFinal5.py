...
group.add(Transformable(Box(pos=(-2.5, 1, 8.1), size=(2.5, 1.75, .5),
              color=GREY_PLASTIC)))
...
group.add(Transformable(Sphere(pos=(4+cos(pi/12)+cos(pi/6), sin(pi/12)+sin(pi/6), 0), radius=1, color=silver)))
...
group.add(g)
g = Transformable(Box(color=Material((.82, .41, .12))))
g.scale(.1, 5, .1).rotate_z(30).translate(4+2*sin(pi/6), 2.5+(2.5-2.5*cos(pi/6)), 0)
group.add(g)
g = Transformable(Box(color=Material((.82, .41, .12))))
