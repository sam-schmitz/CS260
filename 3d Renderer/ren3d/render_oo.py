...
def project_points(points, camera):
    campoints = [p.transform(camera.trans) for p in points]
    d = -camera.distance 
    projpoints = [(d*p.x/p.z, d*p.y/p.z, p.z) for p in campoints]
    return projpoints
...
def render_signature(scene, img):
    ...
    for poly in scene.objects.iter_polygons():
...
def render_wireframe(scene, img):
    ...
    fb = FrameBuffer(img, camera.window)
    for poly in scene.objects.iter_polygons():
        points = project_points(poly.points, camera)
...
def lambert_colors(scene, poly):
    ...
    for pt, norm in zip(poly.points, poly.normals):
        ...
        rgbs.append(poly.color.diffuse * lambert + scene.ambient)
    return rgbs
